...
diagnostic_flags := -fdiagnostics-color=always -Wall -Wextra -Werror -Wno-noexcept-type
cc_specific_flags :=
cxx_specific_flags :=
objc_specific_flags :=
...
ifeq ($(cross_target),msvc)
	...
	cc_specific_flags :=
	cxx_specific_flags := /permissive- /std:c++14
...
ifeq ($(cross_target),darwin)
	...
	cxx_specific_flags += -std=c++14 -stdlib=libc++
